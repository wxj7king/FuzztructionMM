...
{   
    ...
    if (ins_opcode == XED_ICLASS_MOV && INS_OperandIsReg(ins, 0) && INS_OperandIsReg(ins, 1) && insCount < MAX_INSNUM){
        if (isValidIns(ins, INS_TYPE_LOAD_ARGS)){
            ...
            // test
            if (((INS_Address(ins) - 0x28dd) & 0x000000000000ffff) == 0){
                printf("find obj_nid2ln!\n");
                injected += 1;
            }
            else if (((INS_Address(ins) - 0x279a) & 0x000000000000ffff) == 0){
                printf("find obj_nid2sn!\n");
                injected += 1;
            }
            else{
                return;
