...
//VOID InstrumentIns(INS ins, VOID *v)
{
    ...
    while(true){
        ...
        if (shmq.ins_buf.size() == 0){
            PIN_MutexUnlock(&shmq.mutex);
            continue;
        }
        INS ins = shmq.ins_buf.front();
        shmq.ins_buf.pop();
        ...
        if (instmap.count(INS_Address(ins))) {
            PIN_MutexUnlock(&mutex1);
            continue;
        } //FIXME ? Why there could be multiple instrument?
        PIN_MutexUnlock(&mutex1);
    ...
    for (BBL bbl = TRACE_BblHead(trace); BBL_Valid(bbl); bbl = BBL_Next(bbl)) {
        ...
        for(INS ins = BBL_InsHead(bbl); INS_Valid(ins); ins = INS_Next(ins)){
            while(1){
                if (global_exit) return;
                PIN_MutexLock(&shmq.mutex);
                if (shmq.ins_buf.size() > shmq.max_size){
                    PIN_MutexUnlock(&shmq.mutex);
                    continue;
                }
                shmq.ins_buf.push(ins);
...
{   
    ...
    for (int i = 0; i < num_thd; i++)
    {
        ...
        THREADID threadId = PIN_SpawnInternalThread(InstrumentIns, NULL, 0, &threadUid);
